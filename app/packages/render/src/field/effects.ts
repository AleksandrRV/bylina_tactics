...
export function drawFxList(gStatic: Graphics, glowG: Graphics, fxs: Fx[], now: number): void {
  for (let i = fxs.length - 1; i >= 0; i -= 1) {
    ...
    if (fx.kind === "windup") {
      ...
      if (t >= 1) { fxs.splice(i, 1); continue; }
      ...
      const colors: Record<string, number> = {
        POISON: 0x78d83d, PANIC: 0xb94cff, IMMOBILE: 0x709343,
        HIDDEN: 0x78c9b2, FLYING: 0x9edfff, TIMED: 0x5fd6e8,
        DEFENDING: 0x68aee8, OVERWATCH: 0xe8b64c,
      };
      ...
      for (let p = 0; p < 8; p += 1) {
        ...
        g.circle(fx.x + Math.cos(angle) * radius, fx.y + Math.sin(angle) * radius, 2).fill({
          color, alpha: (1 - t) * 0.8 * direction * direction,
        });
      }
      ...
      for (let p = 0; p < 5; p += 1) {
        ...
        g.circle(fx.x + sway, fx.y - 8 - phase * 34, 1.8 * (1 - phase) + 0.6).fill({
          color: 0xf2dd9a, alpha: 0.9 * (1 - phase),
        });
      }
      g.circle(fx.x, fx.y - rise * 0.4, 9 * (1 - t * 0.6)).stroke({
        width: 1.6, color: 0xe8c96a, alpha: 0.8 * (1 - t),
      });
