...
export interface Tile {
  ...
  pit: boolean;
  blockLOS: boolean;
  /** Признак зоны эвакуации: юнит, находящийся в клетке, может выполнить умение с признаком извлечения (§6 game-rules). */
  extract?: boolean;
...
export interface EntityState {
  ...
  /**
   * Талант «Стойка на ходу» (0.21.30): если за свой ход боец только
   * перемещался, в конце хода он сам встаёт в защитную стойку (§15.8).
   */
  autoDefend?: boolean;
  /** Совершал ли боец в текущем ходу действие помимо перемещения (для autoDefend). */
...
export interface MatchState {
  ...
  /**
   * Эвакуированные бойцы высадки (0.13.0): метка высадки и здоровье на момент
...
/**
 * Цель миссии тактического сражения (game-design §3.2).
