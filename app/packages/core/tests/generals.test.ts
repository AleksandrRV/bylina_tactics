...
const YAGA: SpawnUnitConfig = {
  id: "baba_yaga", maxHealth: 24, maxAP: 2, mobility: 6, aim: 80, defense: 5, will: 90,
  vision: 14, weapons: ["branch"], skills: [], tags: ["flying"], fleeHp: 6,
};
const SOLOVEY: SpawnUnitConfig = {
  id: "solovey", maxHealth: 18, maxAP: 2, mobility: 5, aim: 85, defense: 5, will: 60,
  vision: 13, weapons: ["bow"], skills: [], tags: ["hiddenStart"],
};
...
describe("generals (0.18.0)", () => {
  ...
  it("a fleeing general leaves the field without dying", () => {
    ...
    const player = kernel.getSnapshot().entities.find((entity) => entity.owner === PLAYER_OWNER && entity.coverType === 0)!;
    ...
    const applied = kernel2.apply({ type: "ATTACK", actorId: placedPlayer.id, targetId: placedYaga.id, weaponId: "sword" });
  ...
  it("a general killed in the mission is recorded as dead", () => {
    ...
    const applied = kernel.apply({ type: "ATTACK", actorId: placedPlayer.id, targetId: placedYaga.id, weaponId: "sword" });
    expect(applied.ok).toBe(true);
    if (!applied.ok) return;
