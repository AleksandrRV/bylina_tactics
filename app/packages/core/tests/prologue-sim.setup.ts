...
export const MIKULA: SpawnUnitConfig = {
  id: "mikula_peasant",
  maxHealth: 8,
  maxAP: 2,
  mobility: 5,
...
export const RAT: SpawnUnitConfig = {
  id: "forest_rat",
  maxHealth: 4,
  maxAP: 2,
  mobility: 6,
...
export const FEDOT: SpawnUnitConfig = {
  id: "fedot_stranded",
  maxHealth: 5,
  maxAP: 2,
  mobility: 4,
...
export const BOGATYR: SpawnUnitConfig = {
  id: "bogatyr",
  maxHealth: 12,
  maxAP: 2,
  mobility: 4,
...
export const STRELETS: SpawnUnitConfig = {
  id: "strelets",
  maxHealth: 8,
  maxAP: 2,
  mobility: 5,
...
export const ZNAHARKA: SpawnUnitConfig = {
  id: "znaharka",
  maxHealth: 8,
  maxAP: 2,
  mobility: 4,
...
export const CLUB = weaponStatsFromRecord({
  id: "club",
  category: "melee",
  apCost: 1,
  endsTurn: true,
...
export const TEETH = weaponStatsFromRecord({
  id: "teeth",
  category: "melee",
  apCost: 1,
  endsTurn: true,
...
export function m2Setup(options: { adjacent?: boolean } = {}): {
...
export function freeFedot(
  ...
  if (Math.abs(mikula.x - fedot.x) + Math.abs(mikula.y - fedot.y) > 1) {
    const step = kernel
      ...
      .filter((c) => c.apCost === 1 && Math.abs(c.x - fedot.x) + Math.abs(c.y - fedot.y) <= 1)[0]!;
