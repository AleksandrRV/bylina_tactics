...
const testModules = import.meta.env.MODE === "test"
  ? import.meta.glob("../../../packages/content/data/**/*.json5", { eager: true, query: "?raw", import: "default" }) as Record<string, string>
  : null;
...
async function loadFetchedContent(): Promise<ContentLoadResult> {
  ...
  try {
    ...
    return { ok: false, issues: [{ file: "content-manifest.json", message: error instanceof Error ? error.message : String(error) }] };
  }
}
