...
`until: "dash"` завершается перемещением за два очка действия (событие перемещения с признаком рывка). Необязательное поле `notes` миссии задаёт ключи реактивных плашек, показываемых событиями любой стороны в миссии «Умения и состояния» (0.20.1); с доводкой 0.20.2 Навь в миссиях с противником действует, поэтому события отравления и воскрешения реально возникают в бою.

Ограничения: миссии обучения используют обычные записи `units`, `weapons`, `skills` и `map`; подсказки не меняют правила — они только указывают игроку на допустимые действия (ядро отклоняет недопустимые команды как обычно). Интерфейс выполняет подсказки в порядке поля `step` (порядок массива значения не имеет), а завершает шаг только действием игрока — события хода противника подсказки не продвигают. Миссия с непустым `enemies` играется до исхода боя (уничтожение всех противников), миссия без противника завершается последним шагом подсказки (0.20.2). Прогресс обучения хранится в хранилище приложения, не в конфигурации.
