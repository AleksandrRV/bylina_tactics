...
Правила: шаг `fade` обязан нести направление; шаг с целью (`focus`, `pan`) обязан её указать; ровно один источник цели из `cell` / `configId` / `marker`. `marker` разрешается в клетку маркера раскладки интерфейсом — средство отображения о раскладке не знает.
