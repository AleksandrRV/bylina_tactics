...
`until: "dash"` завершается перемещением за два очка действия (событие перемещения с признаком рывка). Необязательное поле `notes` миссии задаёт ключи реактивных плашек, показываемых событиями любой стороны в миссии «Умения и состояния» (0.20.1).
