...
Раздел описывает планируемую конфигурацию режима обучения (версия 0.19.0, план разработки, подэтап 5.2). Схема добавляется вместе с реализацией режима; приведённая ниже форма — нормативное описание поля.
...
Ограничения: миссии обучения используют обычные записи `units`, `weapons`, `skills` и `map`; подсказки не меняют правила — они только указывают игроку на допустимые действия (ядро отклоняет недопустимые команды как обычно). Прогресс обучения хранится в хранилище приложения, не в конфигурации.
