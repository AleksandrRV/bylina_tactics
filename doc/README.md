...
**Текущая версия комплекта: 0.20.49** (совпадает с версией приложения в `app/package.json`). Версия комплекта проверяется скриптом `app/scripts/check-version-consistency.mjs` вместе с визуальным аудитом `audit:visual`; перед публикацией запускайте его из каталога `app`.
