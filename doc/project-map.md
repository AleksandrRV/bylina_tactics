...
| `app/packages/ui/src/battle-match.ts` | Сборка партии для экрана боя: пролог, обучение, повтор, сохранённая партия, поочерёдная игра, кампания, быстрый матч (0.20.68) |
| `app/packages/ui/src/prologue-director.ts` | Режиссёр сцен пролога: кинематографические сцены, выход стаи, зона эвакуации, откат к контрольной точке, сценарий хода героя (0.20.67) |
| `app/packages/ui/src/battle-enemy-phase.ts` | Ход Нави: когда противник ещё ходит, когда круг цикла завершён (0.20.66) |
| `app/packages/ui/src/battle-command.ts` | Единый канал команд: маршрут команды (своё ядро, ведущий, отказ сценария) и итог команды в прологе (0.20.64) |
...
| `app/packages/ui/src/battle-keyboard.ts` | Карта клавиш боевого экрана: разбор нажатия по состоянию в намерение, без React (0.20.59; намерения — 0.20.65) |
| `app/packages/net/src/webrtc.ts` | Канал локальной сети: предложение и ответ, приём чужого описания, предел ожидания сбора кандидатов (0.21.18) |
| `app/packages/ui/src/PvpRoomScreen.tsx` | Комната сбора «Потешных боев»: вкладки «на одном устройстве», «по локальной сети», «сеть общего пользования», набор бойцов (0.21.18) |
