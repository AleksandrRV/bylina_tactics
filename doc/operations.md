...
| `quality` | `pnpm typecheck`, `pnpm test`, `pnpm build` | да |

> **Право `workflows`.** Файл `.github/workflows/ci.yml` подготовлен в
> рабочей копии, но приложение Arena не имеет права `workflows` и не может
> положить его в ветку. Варианты: переподключить GitHub в Arena с этим
> правом или скопировать готовый образец `doc/examples/ci.yml` в
> `.github/workflows/ci.yml` вручную. До этого на ветках проверяется только
> `version-consistency`.

Прежде на ветках проверялась только согласованность версий, а типизация и
613 тестов запускались лишь при публикации в `main` — регрессию было видно
слишком поздно.
